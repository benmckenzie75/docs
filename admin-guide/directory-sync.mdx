...
| **API permissions** | See the table below — all **Application** type, admin-consented |

### API permissions

<Warning>
**Set these as `Application` permissions, not `Delegated` — and click *Grant admin consent* afterwards.**

This is the single most common setup problem, and both ways of getting it wrong fail *quietly*:
...
|---|---|---|
| `User.Read.All` | Application | Users, their profile fields, and their **profile photos** |
| `Group.Read.All` | Application | Listing groups so you can pick one to scope the sync to |
| `GroupMember.Read.All` | Application | Reading who is in that group |
| `Directory.Read.All` | Application | **Optional but recommended** — discovering custom directory extension attributes |

<Info>
The first three are the configuration we test against and the one support will ask you to confirm.

`Directory.Read.All` is optional and fails differently: without it the connection test still passes, but custom extension attributes your organisation defined never appear in the field-mapping list. See [Extension attributes](#extension-attributes).
</Info>
...
Entra shows the two side by side and they look equally like credentials. The **Secret ID** is a GUID and is useless to CaptrID. The **Value** is roughly 40 characters and usually contains a `~`.

The Value is shown **once, at the moment of creation**, and is masked permanently afterwards. There is no way to retrieve it later — if you did not copy it, the only fix is to create a new secret.
</Warning>
...
3. Enter:
   ...
   - **Client Secret** — the secret **Value**, not the Secret ID ([see above](#creating-the-client-secret))
...
|---|---|---|
| **On-premises extension attributes** | `extensionAttribute1` … `extensionAttribute15` | Nothing extra — these arrive with `User.Read.All`. Synced from on-prem AD |
| **Directory extension attributes** | `extension_<appid>_<name>` | `Directory.Read.All`, so CaptrID can discover them |

Both appear under **Extension Attributes** in the mapping wizard once they are visible, and both work as a UID.

<Tip>
`extensionAttribute1`–`15` are the classic place a student or employee number lives in a school that syncs from on-premises Active Directory. If your standard fields have nothing stable to key on, look there before giving up and using `mail`.
</Tip>
...
If you delete a directory connection that a Master List is still syncing from, CaptrID warns you first and names what depends on it.

Deleting stops those lists syncing. The people already imported stay exactly where they are — removing the connection does not remove anyone from your roster.
...
Directory sync **pulls each person's profile photo from Entra** when one is set, so headshots populate automatically alongside their data.

Choose what happens when someone already has a photo:

| Overwrite policy | Behaviour |
|---|---|
| **Only if missing** *(recommended)* | Entra fills gaps and never replaces an existing photo |
| **Always** | Entra's copy wins on each sync |

<Info>
**A photo uploaded by hand is never overwritten by sync — not even on "Always".**

This is deliberate. A manual upload exists to fill a gap the directory could not, and a scheduled sync quietly undoing that work would be the wrong outcome. If you *want* the directory to take that person back over, remove the manual photo and the next sync will supply one.

It is worth knowing, because otherwise "Always" looks broken on exactly the people someone took the trouble to fix.
</Info>

Photos approved in a capture session are pushed to the Master List separately — see [Save to Master List](/admin-guide/saving-changes-to-master-list).
...
|-------|-------|----------|
| `AADSTS7000215` — invalid client secret | The **Secret ID** was pasted instead of the secret **Value**. They sit next to each other in Entra and only the Value works | Create a new client secret and copy the **Value** column immediately — the original Value cannot be recovered once masked |
| Test succeeds but finds **0 users and 0 groups** | Permissions are Delegated rather than Application, or admin consent was never granted | Set all three to **Application** type, click **Grant admin consent**, then test again |
| Sync stopped working after months of running fine | The client secret expired | Create a new secret in the app registration and update it in CaptrID. Nothing else needs changing |
...
| A custom attribute is missing from the field list | The app registration lacks `Directory.Read.All`, so CaptrID could not discover your directory extension attributes. The connection test still passes, which is why this is easy to miss | Add `Directory.Read.All` as an **Application** permission, grant admin consent, then re-run **Test Connection** |
