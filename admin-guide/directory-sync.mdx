...
Grant these on the app registration as **Application** permissions (not Delegated), then click **Grant admin consent**.

| Permission | Required? | What it is used for |
|---|---|---|
| `User.Read.All` | **Required** | Reading users, their profile fields, and their **profile photos** |
| `GroupMember.Read.All` | **Required** | Listing groups and reading their members, so you can scope a sync to one group |
| `Directory.Read.All` | **Recommended** | Discovering **custom directory extension attributes** so they appear as mappable fields |

<Warning>
**If you skip `Directory.Read.All`, the connection test still succeeds.** CaptrID asks Entra for your directory's custom extension properties, and if that call is refused it carries on without them rather than failing the whole test.

The symptom appears later: a custom attribute your organisation defined is simply **missing from the field-mapping list**, with nothing to say why. If you are looking for one and cannot find it, this permission is the first thing to check.
</Warning>
