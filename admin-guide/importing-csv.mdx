...
description: "How to bulk-import people from a CSV or Excel spreadsheet into a Master List or session."
---
...
| Requirement | Detail |
|-------------|--------|
| **Format** | CSV (comma-separated values) **or Excel (`.xlsx`)** — upload either |
| **Encoding** | UTF-8 recommended. A byte-order mark from Excel is handled automatically |
...
<Info>
**Exporting from Excel?** You can upload the `.xlsx` file directly — there is no need to save as CSV first, which is where accented characters and the leading byte-order mark usually get mangled.

If a file cannot be read at all, the import is rejected outright with an explanation rather than importing a partial or garbled roster.
</Info>
