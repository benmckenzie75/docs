...
5. **They're told the outcome automatically** — approved or rejected, by email
...
## What the person hears back

You don't need to chase anyone. When you approve or reject a self-uploaded photo, CaptrID emails the person automatically — they have no account and no portal to check, so email is the only thing that reaches them.

**If you reject it**, the email includes your reason and asks for a new photo. Their original upload link is reopened so it works a second time, meaning they can use the email they already have.

<Info>
Upload links expire (72 hours by default). If you reject a photo after the link has expired, the email tells the person to contact you for a new one rather than sending them to a link that will fail. Send a fresh invitation from the session's self-upload panel.
</Info>

<Tip>
Turn off **Allow Resubmission After Rejection** if you'd rather handle retries yourself — the person is still told the outcome, they're just not given another attempt automatically.
</Tip>
...
<Frame caption="Session self-upload settings showing the email field selector, deadline, and submission options">
  <img src="/images/admin-guide/self-upload-session-config.png" alt="Session configuration panel with self-service upload settings including email field, deadline, and max submissions" />
</Frame>
...
<Frame caption="The self-upload landing page as seen on a mobile device, showing the welcome message and Upload Photo button">
  <img src="/images/admin-guide/self-upload-landing-page.png" alt="Mobile self-upload landing page with organisation name, person greeting, and Upload Photo button" style={{maxWidth: '300px'}} />
</Frame>
...
<Frame caption="Invitation status list showing each person's upload progress with Sent, Opened, and Submitted indicators">
  <img src="/images/admin-guide/self-upload-invitation-status.png" alt="List of self-upload invitations with status badges showing Sent, Opened, Submitted, and Revoked states" />
</Frame>
