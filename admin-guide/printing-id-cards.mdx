...
<Frame caption="The Print Wizard template selection step showing compatible session and organisation templates">
  <img src="/images/admin-guide/print-wizard-template-selection.png" alt="Print Wizard showing a list of compatible card templates with preview thumbnails" />
</Frame>
...
<Frame caption="People selection list with print readiness indicators — green ticks for ready, amber warnings for missing photos">
  <img src="/images/admin-guide/print-wizard-people-selection.png" alt="Print Wizard people list with checkboxes, search, and readiness indicators showing which people are ready to print" />
</Frame>
...
<Frame caption="Card preview showing the front of an ID card rendered with a person's photo, name, and mapped field values">
  <img src="/images/admin-guide/print-wizard-card-preview.png" alt="Print preview showing a rendered ID card with photo, name, student ID, and year level filled in from session data" />
</Frame>
...
Printing a large batch can be interrupted — a paper jam, a closed laptop, a workstation restart. CaptrID does not lose track of where it got to.

When you return, the interrupted batch is **discovered automatically** and you are offered the choice to resume it. You do not have to work out which cards had already printed and re-select the remainder by hand.

<Info>
A card is flagged as **issued** whenever it prints, on any printer — including a print-only job with no encoding. So the issuance record reflects what physically came out of the machine, not only the cards that went through a chip write.
</Info>

## Printing while offline

If the workstation loses its connection mid-run, issuance records are queued locally rather than dropped. They are sent when the connection returns, and again on next start-up, so a card printed during an outage still ends up on the record.

The pending count is shown so you can see there is something waiting rather than assuming it went through.
