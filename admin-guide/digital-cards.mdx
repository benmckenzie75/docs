...
<Info>
New templates default to **Photo ID**, which is the right starting point for most organisations. A **QR code** is optional — turn it on when the pass needs to be scanned, for example to open the holder's [profile page](/admin-guide/hosted-profile-pages) at a door or a front desk.
</Info>
...
### Saving changes to a live template

Saving a template **regenerates every pass already issued from it**. Holders see the new design without doing anything — no reissue, no new link to send.

That is usually what you want, and it is worth knowing before you experiment on a template that three hundred people are already carrying.

<Tip>
**Clone** the template first if you want to try a new design. You get a copy to work on, and the live one keeps serving the passes people already hold.
</Tip>
